...
| delete_scanned_objects          | true     | `True` (**Note:** `metadata` **scan type defaults to** `False`) | Whether to delete scanned objects from source location after copy to relevant S3 bucket.         |                             |         
...
* **Delete scanned objects from source location**: False
...
**Important Note**: tdr-antivirus uses >= v1.9.8 of Terraform. Ensure that Terraform >= v1.9.8 is installed before proceeding.
